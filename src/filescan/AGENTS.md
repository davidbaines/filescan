...
- Include a test that creates synthetic data including duplicate files and folders. Check Excel report contain a row to describe each dupicate.
- Keep the synthetic test data and the test Excel file in 'tests/data' so that manually verification is also possible.
- Maintain comprehensive startup checks and fail early if the configured database is not writable.
