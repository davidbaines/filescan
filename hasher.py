...
class Hasher:
    ...
    def _get_or_compute_hash(self, file_id, path, full=False):
        ...
        if full:
            ...
            if h:
                self.db.upsert_hash(file_id, full_hash=h)
        else:
            h = self._quick_hash(path)
            if h:
                self.db.upsert_hash(file_id, quick_hash=h)
        return h
    ...
    def run(self):
        ...
        quick_matches = []
        for result_id, fa_id, fb_id, fa_path, fb_path, fname, fsize in tqdm(
            pairs, desc="  Quick hash"
        ):
            ...
            if ha == hb:
                quick_matches.append(
                    (result_id, fa_id, fb_id, fa_path, fb_path, fname, fsize)
                )
            else:
                self.db.insert_file_match(result_id, fa_id, fb_id, False, None)
        self.db.commit()
